---
version: alpha
name: CutSteward Dark Professional — Frame (video / frame layer)
description: >
  Premium video-first CutSteward system. The unit is the frame (1920×1080). Atoms are a true-black
  canvas, warm ivory type, antique-gold as the only accent, Inter throughout, restrained translucent
  cards with no content shadows, pill chrome, and finite motion followed by stillness.
...
colors:
  ...
  text-muted: "#B8AD98"
  text-light: "#7B7468"
  accent-light: "rgba(205, 186, 145, 0.08)"
  accent-medium: "rgba(205, 186, 145, 0.15)"
  border: "rgba(205, 186, 145, 0.2)"
  card-bg: "rgba(205, 186, 145, 0.055)"
  positive: "#059669"
  negative: "#dc2626"
...
This is the **blue-professional** structure remixed onto CutSteward's dark captured brand. The YAML frontmatter above (colors · typography · components) is **normative and already correct — use it verbatim.** The prose below is the ORIGINAL preset's intent; read it THROUGH the frontmatter:

- **Fonts** — already set to **Inter** (display) / **Arial** (body); ignore any preset font name lingering in prose.
- **Colors** — use the frontmatter hex; preset color NAMES in prose (e.g. "cobalt", "cream") mean antique gold, true black, and warm ivory from the remapped tokens. Any prose instruction to start on cream means start on the normative black canvas.
